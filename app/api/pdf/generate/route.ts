...
export async function POST(req: NextRequest) {
  try {
    ...
    const routeLabel = '/api/pdf/generate';
    const diagnostics: RouteRenderDiagnostic[] = translatedDocs.map((doc) => {
      ...
      try {
        const renderAssertion = assertStructuredClientFacingRender({
          ...
          translatedText: doc.translatedText ?? undefined,
          surface: routeLabel,
          logPrefix: `[pdf/generate] Order #${orderId} Doc #${doc.id}`,
        });
    ...
    if (unsupportedOrMissing.length > 0) {
      console.error(
        `[pdf/generate] Order #${orderId} blocked: missing structured support | ${summarizeDiagnostics(diagnostics)}`,
      );
      return NextResponse.json(
        {
          success: false,
    ...
    console.error(
      `[pdf/generate] Order #${orderId} blocked: family mismatch (legacy linear route) | ${summarizeDiagnostics(diagnostics)}`,
    );
