...
export const Header = () => {
    ...
    return (
        <motion.header
            ...
            className="fixed top-0 left-0 right-0 z-[100] bg-white/95 backdrop-blur-md border-b border-gray-100 shadow-sm"
        >
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-20 md:h-24 flex items-center justify-between relative z-[101]">
                <div className="flex items-center gap-2 relative z-[102]">
                    <Link href="/" className="cursor-pointer pointer-events-auto transition-transform active:scale-95">
                        <Image
                ...
                <nav className="hidden md:flex items-center gap-6 text-sm font-medium text-slate-600 relative z-[102]">
                    {/* Admin Link - Fixed Path and Interactions */}
                    <Link
                ...
                <div className="md:hidden flex items-center gap-2 relative z-[102]">
                    <button
                        onClick={toggleLocale}
                        className="flex items-center gap-1 px-2.5 py-1.5 rounded-full border border-slate-200 text-xs font-bold text-slate-500 hover:text-[#B8763E] hover:border-[#B8763E] transition-all cursor-pointer pointer-events-auto"
                    >
                    ...
                    <button
                        onClick={toggleMenu}
                        className="p-2 text-slate-600 hover:text-[#B8763E] transition-colors cursor-pointer pointer-events-auto"
                    >
            ...
            <AnimatePresence>
                {isMenuOpen && (
                    <motion.div
                        ...
                        className="md:hidden bg-white border-b border-gray-100 overflow-hidden relative z-[99]"
                    >
                        <nav className="flex flex-col p-4 space-y-3 text-center">
                            {/* Mobile Admin Link - Fixed Path and Close Logic */}
                            <Link
                                href="/admin/dashboard"
